...
function AddLenderSection() {
  ...
  const [result, setResult] = useState(null);
  const [error, setError] = useState(null);
  const [selectedFields, setSelectedFields] = useState({}); // { fieldName: true/false }
  ...
  // Handler: update existing record with only user-selected fields
  const handleUpdateExisting = async (recordId) => {
    if (!result?.extracted) return;

    // Get list of field names the user checked
    ...
    setIsSubmitting(true);
    setError(null);
    setProgress("Updating selected fields...");
    ...
    try {
      ...
      const res = await fetch(ADD_LENDER_URL, {
        ...
        body: JSON.stringify({
          url: url.trim(),
          update_existing: recordId,
          selected_fields: fieldsToUpdate,
        }),
      });
  ...
  return (
    <div className="mt-10">
      ...
      <div className="bg-surface border border-border-light rounded-xl p-6 mb-6 shadow-sm">
        ...
        {/* Duplicate Warning — with comparison table and checkboxes */}
        {result && result.duplicate && (
          <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
              ...
              <div className="mb-4">
                ...
                <div className="overflow-x-auto rounded-lg border border-amber-200">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="bg-amber-100 text-amber-900">
                        <th className="px-2 py-2 text-left w-8">
                          <input
                            type="checkbox"
                            className="rounded border-amber-300 cursor-pointer"
                            checked={
                              Object.entries(result.comparison)
                                .filter(([, v]) => v.new && v.new !== v.current)
                                .every(([field]) => selectedFields[field])
                            }
                            onChange={(e) => {
                              const newSelected = { ...selectedFields };
                              for (const [field, vals] of Object.entries(result.comparison)) {
                                if (vals.new && vals.new !== vals.current) {
                                  newSelected[field] = e.target.checked;
                                }
                              }
                    ...
                    <tbody>
                      {Object.entries(result.comparison).map(([field, vals]) => {
                        const hasNew = vals.new && vals.new !== vals.current;
                        const isSame = vals.new === vals.current;
                        return (
                          <tr
                            key={field}
                            className={`border-t border-amber-100 ${
                              isSame ? "opacity-40" : hasNew && selectedFields[field] ? "bg-blue-50/50" : ""
                            }`}
                          >
                            ...
                            <td className="px-2 py-2 font-semibold text-text-muted whitespace-nowrap">
                              {field}
                            </td>
                            <td className="px-2 py-2 text-text break-all max-w-[250px]">
                              {vals.current ? (
                                typeof vals.current === "string" && vals.current.startsWith("http") ? (
                                  <a href={vals.current} target="_blank" rel="noopener noreferrer" className="text-primary-600 no-underline hover:underline">
                                    {vals.current.length > 60 ? vals.current.slice(0, 60) + "..." : vals.current}
                                  </a>
                                ) : (
                                  String(vals.current).length > 100 ? String(vals.current).slice(0, 100) + "..." : String(vals.current)
                                )
                              ) : (
                            ...
                            <td className="px-2 py-2 text-text break-all max-w-[250px]">
                              {vals.new ? (
                                typeof vals.new === "string" && vals.new.startsWith("http") ? (
                                  <a href={vals.new} target="_blank" rel="noopener noreferrer" className="text-primary-600 no-underline hover:underline">
                                    {vals.new.length > 60 ? vals.new.slice(0, 60) + "..." : vals.new}
                                  </a>
                                ) : (
                                  String(vals.new).length > 100 ? String(vals.new).slice(0, 100) + "..." : String(vals.new)
                                )
                              ) : (
                                <span className="text-gray-300 italic">empty</span>
                              )}
                              {isSame && <span className="ml-1.5 text-[0.625rem] text-gray-400">(same)</span>}
                            </td>
                          </tr>
                        );
                      })}
            ...
            <div className="flex gap-2 flex-wrap">
              {/* Update Selected — sends only checked fields */}
              {result.existing_records?.length > 0 && (
                <button
                  onClick={() => handleUpdateExisting(result.existing_records[0].id)}
                  disabled={isSubmitting || Object.values(selectedFields).filter(Boolean).length === 0}
                ...
                >
                  ...
                  Update Selected ({Object.values(selectedFields).filter(Boolean).length})
                </button>
              )}
              <button
              ...
              <button
                onClick={() => { setResult(null); setSelectedFields({}); }}
