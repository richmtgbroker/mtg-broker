<!-- ================================================================
     PRODUCTS PAGE - JS EMBED (v3.0)
     Page: /app/products
     Updated: March 22, 2026

     CHANGES FROM v2.5 (API-DRIVEN — NO MORE CMS):
     - REMOVED: All CMS/DOM scraping (collectPageOneProducts, loadRemainingPages,
       extractCardData, fetchPage — no more DOMParser or pagination HTML fetches)
     - NEW: Single API fetch from mtg-broker-api Cloudflare Worker
       endpoint: GET https://mtg-broker-api.rich-e00.workers.dev/api/products-list
     - NEW: localStorage cache with 30-minute TTL (cache key: mtg_products_v5)
     - KEPT: All v2.5 features — search, category pills, A-Z alphabet nav,
       count badge, 2-column card grid, letter group headers, print list button,
       loading skeletons, empty state
     - Product cards link to /app/products/{slug}

     WEBFLOW NOTES:
     - The CMS Collection List is no longer needed. You can delete it from
       the Designer or leave it hidden (CSS already hides it with display:none).
     - No changes needed to CSS or HTML embeds.

     PLACE IN: Navigator > JS embed element on Products page
     ================================================================ -->
...
(function () {
  ...
  /* ================================================
     ...
     Replaces CMS DOM scraping from v2.x.
     First checks localStorage for a fresh cached copy.
     Falls back to the Cloudflare Worker API.
     ================================================ */
  async function loadProducts() {
    ...
    try {
      ...
      if (cached) {
        ...
        if (parsed && parsed.ts && (Date.now() - parsed.ts < CACHE_TTL)) {
          console.log('⚡ Products cache hit — ' + parsed.products.length + ' products from localStorage');
    ...
    try {
      ...
      removeLoadingState();
      // Show error in place of loading skeleton
  ...
  function onProductsLoaded(products) {
    // Build allProducts with the shape the UI expects
    ...
    console.log('✅ Products page: rendered ' + allProducts.length + ' products');
  }
  ...
  /* ================================================
     ...
     Prints the currently visible/filtered product list
     with MtgBroker branding at the top
     ================================================ */
  ...
  /**
   * handlePrintList()
   * 1. Gets the currently filtered products
   * 2. Builds a hidden print container with logo + product list
   * 3. Triggers window.print()
   * 4. Cleans up the print container after printing
   */
  function handlePrintList() {
    /* --- Step 1: Get currently filtered products --- */
    ...
    /* --- Step 2: Remove any existing print container --- */
    var existing = document.querySelector('.products-print-container');
    if (existing) existing.parentNode.removeChild(existing);

    /* --- Step 3: Build the print container HTML --- */
    ...
    /* Build the product list HTML grouped by letter */
    ...
    /* --- Step 4: Append to body and print --- */
    document.body.appendChild(container);

    /* Small delay to let the browser render the container before printing */
    setTimeout(function () {
      window.print();
  ...
  /**
   * getFilteredProducts()
   * Returns the array of products matching current filters.
   */
  ...
  /* ================================================
     ...
     Product must contain ALL selected tags
     ================================================ */
  ...
  function renderProducts() {
    ...
    var filtered = allProducts.filter(function (p) {
      // Search filter
      if (searchTerm && p.sortName.indexOf(searchTerm) === -1) return false;
      // Category filter — AND logic: product must have ALL selected tags
      if (activeCategories.length > 0) {
        for (var i = 0; i < activeCategories.length; i++) {
          if (p.tags.indexOf(activeCategories[i]) === -1) return false;
        }
    ...
    filtered.forEach(function (product) {
      // Letter group header (letter + dividing line)
  ...
  /* ================================================
     BUILD CARD (name + chevron, links to product slug)
     ================================================ */
